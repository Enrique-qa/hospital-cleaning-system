...
export function EntityDetailsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setMonitoringStatus(response.data.monitoringStatus);
      setLoading(false);
    }
              ...
              <div>
                ...
                <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                  <Link
                    ...
                    className="inline-flex justify-center rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white"
                  >
                  ...
                  <Link
                    ...
                    className="inline-flex justify-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700"
                  >
                    Editar entidade
                  </Link>
                  ...
                  <Link
                    ...
                    className="inline-flex justify-center rounded-xl border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-700"
                  >
                    Imprimir etiqueta
                  </Link>
                </div>
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
            ...
            {monitoringStatus && (
              <div
                className={
                  monitoringStatus.status === "PENDING"
                    ? "rounded-xl bg-red-50 px-4 py-3 text-red-800"
                    : monitoringStatus.status === "OK"
                      ? "rounded-xl bg-green-50 px-4 py-3 text-green-800"
                      : "rounded-xl bg-slate-100 px-4 py-3 text-slate-700"
                }
              >
                <p className="text-sm font-bold">{monitoringStatus.label}</p>
                <p className="mt-1 text-xs">{monitoringStatus.reason}</p>
                <p className="mt-2 text-xs font-semibold">
                  Última limpeza:{" "}
                  {monitoringStatus.lastCleaningAt
                    ? new Date(
                        monitoringStatus.lastCleaningAt
